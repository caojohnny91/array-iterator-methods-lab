...
let veryOldInventors =  inventors.filter((born1500s) =>{
    return born1500s.year >= 1500 && born1500s.year <= 1599;
});
...
let sortedByBirthYear = inventors.slice().sort((a, b) => {
...
const ageIs18OrOlder = currentYear - 18;

let isAdultPresent = devs.some((adult) =>{
    return adult.year <= ageIs18OrOlder;
});
...
const ageIs19OrOlder = currentYear - 19

let isEveryone19OrOlder = devs.every((nineteenOrOlder) =>{
    return nineteenOrOlder <= ageIs19OrOlder;
});
...
let totalYearsLived = inventors.reduce((totalYears, inventor) =>{
